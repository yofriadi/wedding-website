...
- FROM: `### Requirement: Submissions API never cached; photo files cacheable`
- TO: `### Requirement: Guest photo API never cached; photo files cacheable`
- FROM: `### Requirement: One submission per invite`
- TO: `### Requirement: One photo per invite`
- FROM: `### Requirement: Submission requires photos`
- TO: `### Requirement: Upload requires one photo`

> Sync disposition: Apply these renames before the MODIFIED blocks, which use the final titles. Explicit scenario-removal notes below are instructions for the spec merge, not requirement text to copy into the main spec.
...
> Sync disposition: Explicitly remove the superseded scenario **"One bad file fails the batch"**. The seven scenarios below are this requirement's complete final scenario set; do not retain the former multi-photo batch or claim-first cleanup scenario.
...
> Sync disposition: Explicitly remove the superseded scenario **"Submissions API never cached"**, including when found under this requirement's pre-rename title. The three scenarios below are the complete final scenario set; no scenario requiring `GET /api/submissions` survives.
...
> Sync disposition: Replace the former scenario **"Empty submission rejected"** with **"Empty upload rejected"** below. This is the complete final scenario set for the renamed requirement; do not preserve a duplicate scenario under the retired title.
...
**Reason**: A single photo has no submission parent or position; the fresh setup does not retain legacy URLs.
**Migration**: Use the photo-scoped immutable storage requirement and start with empty guest-upload storage; do not convert or serve old paths.

#### Scenario: Fresh upload has no submission directory

- **WHEN** a photo is accepted under the replacement model
- **THEN** it uses `guest-photos/<photo-id>/photo.webp` and no `submissions/` directory is created

### Requirement: Photos served via the wall

**Reason**: The story wall and attributed rail have been replaced by the magnetic image trail.
**Migration**: Read the flat `/api/guest-photos` collection and render it through `guest-photo-trail`; keep validated public file serving.

#### Scenario: Public visitor reads the new collection

- **WHEN** a public visitor loads the memories section
- **THEN** guest photos come from the new flat collection without story tiles or a `wall.stories` contract

### Requirement: Guest tiles render inside the rail container

**Reason**: There is no horizontal story rail or dynamically appended guest tile set.
**Migration**: Feed source URLs into `MagneticImageTrail` and preserve all guest sources on refresh.

#### Scenario: Post updates the trail

- **WHEN** an upload is accepted
- **THEN** the new photo enters the trail pool rather than a `data-story-rail` container

### Requirement: Invitee's own tile renders on initial load

**Reason**: Ownership is now identified by `mineId` within one complete photo list, not a separate story tile.
**Migration**: Use the caller's existing photo in the same trail source pool on initial load.

#### Scenario: Returning poster has no own-story tile

- **WHEN** a posted invitee reloads
- **THEN** their photo is present in the trail without constructing an own-story tile

### Requirement: Wall payload carries poster first name and timestamp

**Reason**: The trail needs no public attribution or story grouping; creation time belongs directly to each photo.
**Migration**: Return photo IDs, URLs, and timestamps only, with caller ownership represented by `mineId`.

#### Scenario: Flat payload excludes attribution

- **WHEN** the collection is returned
- **THEN** photos carry `createdAt` but no `firstName`, `mine` object, or `wall` object

### Requirement: First-name attribution on story tiles

**Reason**: Neither story tiles nor their author-header modal exists in the replacement UI.
**Migration**: Retain invitation names for invitation/RSVP features only; the public trail displays images without story author labels.

#### Scenario: Names do not become photo labels

- **WHEN** an invite's display name is changed
- **THEN** the flat photo collection and trail remain free of public first-name attribution
